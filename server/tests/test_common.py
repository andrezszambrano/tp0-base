...
class TestUtils(unittest.TestCase):
    ...
    def test_bet_init_must_keep_fields(self):
        b = Bet(1, 'first', 'last', 10000000, datetime.date(2000, 12, 20), 7500)
    ...
    def test_has_won_with_winner_number_must_be_true(self):
        b = bet(1, 'first', 'last', 10000000, datetime.date(2000, 12, 20), LOTTERY_WINNER_NUMBER)
        self.assertTrue(has_won(b))

    def test_has_won_with_winner_number_must_be_true(self):
        b = Bet(1, 'first', 'last', 10000000, datetime.date(2000, 12, 20), LOTTERY_WINNER_NUMBER + 1)
        self.assertFalse(has_won(b))

    def test_store_bets_and_load_bets_keeps_fields_data(self):
        to_store = [Bet(1, 'first', 'last', 10000000, datetime.date(2000, 12, 20), 7500)]
        store_bets(to_store)
    ...
    def test_store_bets_and_load_bets_keeps_registry_order(self):
        to_store = [
            Bet(0, 'first_0', 'last_0', 10000000, datetime.date(2000, 12, 20), 7500),
            Bet(1, 'first_1', 'last_1', 10000001, datetime.date(2000, 12, 21), 7501),
        ]
        store_bets(to_store)
        from_load = list(load_bets())
